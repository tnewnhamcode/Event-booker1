...
class users (AbstractUser):
    ...
    mobile=models.IntegerField(unique=True, default='00') #have to add a default because its an integer and they need  for the tables already made coz they can't be blank for integers 
    groups = models.ManyToManyField(  #defining relationship to imported model 'group' which means which group users belong to (different groups get different permissions), through a groups field
        ...
        blank=True,     #saying it doesnt need to be filled in  by them
        help_text=(
...
#    update_profile_picture=models.ImageField(default='default.jpg', upload_to='profile_pics') #i think this is coz the database cant store the image, s oinstead stores refereences to the folder on your computer which stores it
